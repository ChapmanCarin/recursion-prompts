...
var range = function(x, y) {
  ...
  if(y - x === 2) return [x + 1];
  if(x - y === 2) return [x - 1];
  else if(y > x) return [x + 1].concat(range (x + 1, y));
  else return [x - 1].concat(range (x - 1, y));
 //return result.push(x).concat(range(x + 1));
};
...
var reverse = function(string) {
  ...
  /*if(string.length === 1) return string[string.length -1];
...
var compareStr = function(str1, str2) {
  if(!str1.length && str1 === str2) return true;
...
var replaceKeysInObj = function(obj, key, newKey) {
  for(let prop in obj){
    ...
    if(typeof obj[prop] === 'object'){
      return replaceKeysInObj(obj[prop]);
    }
  }
  return obj;
};
...
var fibonacci = function(n) {
  ...
  return result.push(fibonacci(n - 1) + fibonacci(n - 2));
...
var nestedEvenSum = function(obj) {
    ...
    for(let key in obj){
      ...
      return nestedEvenSum(obj.obj[key]);
    }
    }
     return result;
};
...
var augmentElements = function(array, aug) {
  ...
  let final = [result].concat(augmentElements(array.slice(1), aug));
